...
class GIN_NodeWeightEncoder(torch.nn.Module):
    def __init__(self, num_features, dim, add_mask=True):
        super().__init__()

        nn1 = Sequential(Linear(num_features, dim), ReLU(), Linear(dim, dim))
        self.conv1 = WGINConv(nn1)
        self.bn1 = torch.nn.BatchNorm1d(dim)

        nn3 = None
        if add_mask == True:
            nn3 = Sequential(Linear(dim, dim), ReLU(), Linear(dim, 3))
            self.conv3 = WGIN(nn3)
            self.bn3 = torch.nn.BatchNorm1d(3)
        else:
            nn3 = Sequential(Linear(dim, dim), ReLU(), Linear(dim, 2))
            self.conv3 = WGIN(nn3)
